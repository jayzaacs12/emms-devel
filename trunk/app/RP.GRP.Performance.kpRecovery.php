...
switch ($_GET['cycle']) {
  ...
  case 'MONTH'	: 
    ...
    $mrow   = WEBPAGE::$dbh->getAll(sprintf('select 1000*YEAR(p.date)+MONTH(p.date) as date, sum(p.principal) as kp, lt.borrower_type from tblPayments as p, tblLoans as l, tblLoanTypes as lt where lt.id = l.loan_type_id and l.id = p.loan_id and 1000*YEAR(p.date)+MONTH(p.date) >= "%s" and 1000*YEAR(p.date)+MONTH(p.date) <= "%s" and %s = %s group by date, lt.borrower_type',$xrange[0]['date'],$xrange[count($xrange)-1]['date'],$_GET['mode'],$_GET[$_GET['mode']]));
    $xmin   = sprintf('%s/%s',$xrange[0]['date']-1000*floor($xrange[0]['date']/1000),floor($xrange[0]['date']/1000));
    $xmax   = sprintf('%s/%s',$xrange[count($xrange)-1]['date']-1000*floor($xrange[count($xrange)-1]['date']/1000),floor($xrange[count($xrange)-1]['date']/1000));
    break; 
  case 'YEAR'	: 
    ...
    $mrow   = WEBPAGE::$dbh->getAll(sprintf('select YEAR(p.date) as date, sum(p.principal) as kp, lt.borrower_type from tblPayments as p, tblLoans as l, tblLoanTypes as lt where lt.id = l.loan_type_id and l.id = p.loan_id and YEAR(p.date) >= "%s" and YEAR(p.date) <= "%s" and %s = %s group by date, lt.borrower_type',$xrange[0]['date'],$xrange[count($xrange)-1]['date'],$_GET['mode'],$_GET[$_GET['mode']]));
    $xmin   = $xrange[0]['date'];
...
foreach ($mrow as $key=>$val) {
  $data[$_LABELS[sprintf('tblLoanTypes.borrower_type.%s',$val['borrower_type'])]][$val['date']] = $val['kp'];
  $data[$_LABELS['total']][$val['date']] += $val['kp'];
  }
...
$graph = New Graph('',$_LABELS[$_GET['cycle']],sprintf('%s (%s)',$_LABELS['RP.GRP.Performance.kpRecovery'],'$'),$dataf,$options);		
