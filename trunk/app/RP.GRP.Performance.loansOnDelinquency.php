...
switch ($_GET['cycle']) {
  case 'DAY'	: 
    ...
    $mrow   = WEBPAGE::$dbh->getAll(sprintf('select 1000*YEAR(lod.date)+DAYOFYEAR(lod.date) as date, count(lod.id) as loansOnDelinquency, lt.borrower_type from tblLoansOnDelinquency as lod, tblLoans as l, tblLoanTypes as lt where lt.id = l.loan_type_id and l.id = lod.loan_id and 1000*YEAR(lod.date)+DAYOFYEAR(lod.date) >= "%s" and 1000*YEAR(lod.date)+DAYOFYEAR(lod.date) <= "%s" and %s = %s group by date, lt.borrower_type',$xrange[0]['date'],$xrange[count($xrange)-1]['date'],$_GET['mode'],$_GET[$_GET['mode']]));
    $xmin   = $_GET['dateFrom'];
    $xmax   = $_GET['dateTo'];
    break; 
  case 'WEEK'	: 
    ...
    $mrow   = WEBPAGE::$dbh->getAll(sprintf('select 1000*YEAR(lod.date)+WEEK(lod.date,"%s") as date, count(lod.id)/7 as loansOnDelinquency, lt.borrower_type from tblLoansOnDelinquency as lod, tblLoans as l, tblLoanTypes as lt where lt.id = l.loan_type_id and l.id = lod.loan_id and 1000*YEAR(lod.date)+WEEK(lod.date,"%s") >= "%s" and 1000*YEAR(lod.date)+WEEK(lod.date,"%s") <= "%s" and %s = %s group by date, lt.borrower_type',$_GET['dateFrom'],$_GET['dateFrom'],$xrange[0]['date'],$_GET['dateFrom'],$xrange[count($xrange)-1]['date'],$_GET['mode'],$_GET[$_GET['mode']]));
  ...
  case 'MONTH'	: 
    ...
    $mrow   = WEBPAGE::$dbh->getAll(sprintf('select 1000*YEAR(lod.date)+MONTH(lod.date) as date, count(lod.id)/30 as loansOnDelinquency, lt.borrower_type from tblLoansOnDelinquency as lod, tblLoans as l, tblLoanTypes as lt where lt.id = l.loan_type_id and l.id = lod.loan_id and 1000*YEAR(lod.date)+MONTH(lod.date) >= "%s" and 1000*YEAR(lod.date)+MONTH(lod.date) <= "%s" and %s = %s group by date, lt.borrower_type',$xrange[0]['date'],$xrange[count($xrange)-1]['date'],$_GET['mode'],$_GET[$_GET['mode']]));
    $xmin   = sprintf('%s/%s',$xrange[0]['date']-1000*floor($xrange[0]['date']/1000),floor($xrange[0]['date']/1000));
    $xmax   = sprintf('%s/%s',$xrange[count($xrange)-1]['date']-1000*floor($xrange[count($xrange)-1]['date']/1000),floor($xrange[count($xrange)-1]['date']/1000));
    break; 
  case 'YEAR'	: 
    ...
    $mrow   = WEBPAGE::$dbh->getAll(sprintf('select YEAR(lod.date) as date, count(lod.id)/365 as loansOnDelinquency, lt.borrower_type from tblLoansOnDelinquency as lod, tblLoans as l, tblLoanTypes as lt where lt.id = l.loan_type_id and l.id = lod.loan_id and YEAR(lod.date) >= "%s" and YEAR(lod.date) <= "%s" and %s = %s group by date, lt.borrower_type',$xrange[0]['date'],$xrange[count($xrange)-1]['date'],$_GET['mode'],$_GET[$_GET['mode']]));
    $xmin   = $xrange[0]['date'];
...
foreach ($mrow as $key=>$val) {
  $data[$_LABELS[sprintf('tblLoanTypes.borrower_type.%s',$val['borrower_type'])]][$val['date']] = $val['loansOnDelinquency'];
  $data[$_LABELS['total']][$val['date']] += $val['loansOnDelinquency'];
  }
...
$graph = New Graph('',$_LABELS[$_GET['cycle']],$_LABELS['RP.GRP.Performance.loansOnDelinquency'],$dataf,$options);		
