...
foreach ($mrow as $key=>$val) {
  $data[$_LABELS[sprintf('tblLoanTypes.borrower_type.%s',$val['borrower_type'])]][$val['date']] = $val['insurances'];
  $data[$_LABELS['total']][$val['date']] += $val['insurances'];
  }
...
$graph = New Graph('',$_LABELS[$_GET['cycle']],sprintf('%s (%s)',$_LABELS['RP.GRP.Performance.insurancesRecovery'],'$'),$dataf,$options);		
