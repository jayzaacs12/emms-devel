...
<title>
Esperanza Internacional - Advisor Performance
</title>
<style type="text/css">
a.hide {display:none;}
</style>
<body>
<form action="index.advisor.performance1.php" method="get">
<table><tr><td>
<select id="report" name="report">
    <option id="00" value="">Seleccione Reporte</option>
    <option id="01" value="01"> Monto Desembolsado</option>
    <option id="02" value="02"> *Riesgo</option>
    <option id="03" value="03"> % Castigo</option>
    <option id="04" value="04"> *Cantidad de Prestamos</option>
    <option id="07" value="07"> % Mujeres</option>
    <option id="08" value="08">Clientes con Prestamos</option>
    <option id="09" value="09">*Cartera Vigente</option>
    <option id="10" value="10">Ingresos Operativos</option>
    <option id="11" value="11">*Cartera Clientes</option>
    </select>
...
if($_GET['report']=="extra4") {
$new_clients_real = WEBPAGE::$dbh->getAll(sprintf("select u.id,concat(u.first,' ',u.last) as asesor,tcp.clients from tblClientPortfolio tcp,tblUsers u where u.id = tcp.advisor_id and tcp.date = '%s' order by u.id;",$end_date));

$_html = "<h1>New Clients Real</h1>";
$_html .= count($new_clients_real) ? WEBPAGE::printchart($new_clients_real,array_keys($new_clients_real[0])) : $_LABELS['noData'];
print $_html;
}
...
FROM    (tblLoans l,
        tblUsers u,(select rp.advisor_id as asesor,sum(rp.balance) as cartera from tblRiskPortfolio rp where rp.date='%s' group by rp.advisor_id) as balance)
...
GROUP BY u.id ORDER BY castigo_percent asc;",$end_date,$start_date,$end_date));
...
$ingresos_operativos = WEBPAGE::$dbh->getAll(sprintf("SELECT  u.id,concat(u.first,' ',u.last) as asesor,
        (SUM(tp.penalties) + SUM(tp.interest) + SUM(tp.fees)) as ingresos        
FROM    (tblLoans l    ,
        tblUsers u)    
...
if($_GET['report']=="04") {
$disbursed_loans = WEBPAGE::$dbh->getAll(sprintf("select u.id,concat(u.first,' ',u.last) as asesor,count(lsh.id) as cantidad_prestamos from tblLoanStatusHistory lsh,tblLoans l,tblUsers as u
 where u.id = l.advisor_id and l.id = lsh.loan_id and lsh.date >= '%s'
 and lsh.date <= '%s' and lsh.status='G' group by u.id order by cantidad_prestamos desc;",$start_date,$end_date));
...
if($_GET['report']=="09") {
$cartera_vigente = WEBPAGE::$dbh->getAll(sprintf("SELECT  u.id,
        concat(u.first,' ',u.last)     AS asesor,
        sum(rp.balance) as cartera_vigente,
        rp.date
FROM    tblRiskPortfolio rp,
        tblUsers u
WHERE   u.id    = rp.advisor_id
    AND rp.date = '%s'
GROUP BY u.id
